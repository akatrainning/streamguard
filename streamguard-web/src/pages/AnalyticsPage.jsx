...
const tipStyle = { background:"rgba(2,8,16,0.92)", border:"1px solid rgba(0,255,224,0.2)", borderRadius:"8px", color:"rgba(228,240,255,0.8)", fontSize:"11px" };
  ...
  return (
    <motion.div initial={{ opacity:0, y:18 }} animate={{ opacity:1, y:0 }} transition={{ delay, duration:0.45 }}
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"12px" }}>
        ...
        <span style={{ fontSize:"9px", color:"rgba(228,240,255,0.3)", letterSpacing:"1px", fontFamily:"Inter" }}>{label}</span>
      </div>
      <div className="mono" style={{ fontSize:"30px", fontWeight:700, color, lineHeight:1, marginBottom:"5px" }}>{value}</div>
      <div style={{ fontSize:"10px", color:"rgba(228,240,255,0.3)" }}>{sub}</div>
    </motion.div>
  );
}
...
function Panel({ title, children, style={} }) {
  return (
    <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(0,255,224,0.08)",
      ...
      <div style={{ fontSize:"10px", letterSpacing:"2px", color:"rgba(228,240,255,0.3)", marginBottom:"14px" }}>{title}</div>
      {children}
    </div>
  );
}
  ...
  return (
    <div style={{ padding:"24px", maxWidth:"1200px", margin:"0 auto" }}>
      <div style={{ marginBottom:"22px" }}>
        ...
        <div style={{ fontSize:"12px", color:"rgba(228,240,255,0.4)", marginTop:"4px" }}>近 7 日综合分析报告</div>
      </div>

      {/* KPI row */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"14px", marginBottom:"20px" }}>
        <KpiCard icon="📊" label="分析场次" value="8"           sub="近7日"        color="#00FFE0" delay={0.05}/>
        ...
        <Panel title="近 7 日理性指数趋势">
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={TREND}>
              ...
              <XAxis dataKey="day" tick={{ fill:"rgba(228,240,255,0.4)", fontSize:10 }} tickLine={false} axisLine={false}/>
              <YAxis domain={[0,100]} tick={{ fill:"rgba(228,240,255,0.3)", fontSize:9 }} tickLine={false} axisLine={false}/>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"5px", marginTop:"6px" }}>
            {PIE.map((d,i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:"6px" }}>
                ...
                <span style={{ fontSize:"10px", color:"rgba(228,240,255,0.5)", flex:1 }}>{d.name}</span>
        ...
        <Panel title="近 7 日话术类型分布（堆叠）">
          <ResponsiveContainer width="100%" height={170}>
            <BarChart data={TREND} margin={{ top:5, right:20, bottom:5, left:-10 }}>
              ...
              <Tooltip contentStyle={tipStyle}/>
              <Legend iconSize={8} wrapperStyle={{ fontSize:"10px", color:"rgba(228,240,255,0.4)" }}/>
        ...
        <Panel title="综合风险维度均值">
          <ResponsiveContainer width="100%" height={170}>
            <RadarChart data={RADAR_AVG} margin={{ top:10, right:20, bottom:10, left:20 }}>
              ...
              <PolarAngleAxis dataKey="subject" tick={{ fill:"rgba(228,240,255,0.4)", fontSize:9 }}/>
